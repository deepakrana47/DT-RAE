...
# def adam_weight_update(w1, w2, grads, grads2, b1=0.9, b2=0.99, e=1e-8, e2=1e-8):
#
#     fix1 = (1. - b1)
#     fix2 = (1. - b2)
#     Global.lr = Global.lr * (np.sqrt(fix2) / fix1)
#     for wpi, gs in grads.items():
#         g=gs[0]
#         for i in gs[1:]:
#             g = g+i
#         g=g.transpose()/len(gs)
#         m = Global.m[wpi]
#         v = Global.v[wpi]
#         m_t = (b1 * g) + ((1. - b1) * m)
#         v_t = (b2 * np.square(g)) + ((1. - b2) * v)
#         g_t = m_t / (np.sqrt(v_t) + e)
#         w1[wpi] = w1[wpi] - (Global.lr * g_t)
#         Global.m[wpi] = m_t
#         Global.v[wpi] = v_t
#
#     fix1 = (1. - b1)
#     fix2 = (1. - b2)
#     Global.lr2 = Global.lr2 * (np.sqrt(fix2) / fix1)
#     g2 = grads2[0]
#     for i in grads2[1:]:
#         g2 = g2 + i
#     g2 = g2.transpose() / len(grads2)
#     m2 = Global.m2
#     v2 = Global.v2
#     m2_t = (b1 * g2) + ((1. - b1) * m2)
#     v2_t = (b2 * np.square(g2)) + ((1. - b2) * v2)
#     g2_t = m2_t / (np.sqrt(v2_t) + e2)
#     w2 = w2 - (Global.lr2 * g2_t)
#     Global.m2 = m2_t
#     Global.v2 = v2_t
#
#     return w1, w2
...
def adam_weight_update(w1, w2, grads, grads2, b1=0.9, b2=0.99, e=1e-8, e2=1e-8):
    ...
    for wpi, gs in grads.items():
        ...
        v_t = (b2 * np.square(g)) + ((1. - b2) * v)
        g_t = m_t / (np.sqrt(v_t) + e)
        w1[wpi] = w1[wpi] - (Global.lr[wpi] * g_t)
        Global.m[wpi] = m_t
        Global.v[wpi] = v_t
        Global.lr[wpi] = Global.lr[wpi] * (np.sqrt(fix2) / fix1)
...
def rae_operation(vect1, w1, w2, w_size, h_vect, h_index, dep_order, Word_ids,hh_index, wp, p, regu, wpresent):
    ...
    for i in range(len(h_vect)):
        # ch_sum = sum(ch_no[i])
        for j in range(len(h_vect[i])):
            temp = np.dot(grad1[h_vect[i][j]], vect21_[w_size + i].transpose())#*ch_no[i][j]/ch_sum
            dw1[wp[i][j]] += temp
            epnum1[wp[i][j]]+=1
            if wp[i][j] in grads:
                grads[wp[i][j]].append(temp)
            else:
                grads[wp[i][j]] = [temp]
    for i in range(len(grad2)):
        temp = np.dot(grad2[i], v[i].transpose())
        dw2 += temp
        epnum2+=1
        grads2.append(temp)
    return vect1_, dw1, dw2, epnum1, epnum2, grads, grads2
...
def rae_trainning_normal(wd_extra, w1, w2, neta, regu=None, wpresent=None):
    ...
    for iword in wd_extra:
        try:
            ...
            vect_, dw1, dw2, epnum1, epnum2, grads, grads2 = rae_operation(vect1=vect, w1=w1, w2=w2, w_size=iword['w_size'], h_vect=iword['h_vect'], Word_ids= iword['Word_ids'], wp=iword['wp'], h_index=iword['h_index'], dep_order = iword['dep_order'], hh_index=iword['hh_index'], p=iword['p'], regu = regu, wpresent=wpresent)
            terr = 0.0
            bc=0
    ...
    w1, w2 = adam_weight_update(w1=w1, w2=w2, grads = grads, grads2=grads2, b1=regu[0], b2=regu[1])
