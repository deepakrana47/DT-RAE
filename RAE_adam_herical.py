...
# def _adam_weight_update(w, grads, epnum, neta, regu, wpresent,  lr=0.0002, b1=0.9, b2=0.99, e=1e-8):
#     i_t = 1.0
#     fix1 = (1. - b1)
#     fix2 = (1. - b2)
...
#         w[wpi] = w[wpi] - (Global.lr * g_t)
#         Global.m[wpi] = m_t
#         Global.v[wpi] = v_t
#     return w

def adam_weight_update(w, grads, epnum, neta, regu, wpresent,  lr=0.0002, b1=0.9, b2=0.99, e=1e-8):
    i_t = 1.0
    fix1 = (1. - b1)
    fix2 = (1. - b2)
    for wpi, gs in grads.items():
...
def rae_operation(vect, w, w_size, h_vect, h_index, dep_order, Word_ids,hh_index, wp, p, neta, regu, wpresent):
    ...
    for i in range(len(h_vect)):
        for j in range(len(h_vect[i])):
            temp = np.dot(grad[h_vect[i][j]], vect_[w_size + i].transpose())
            dw[wp[i][j]] += temp
            epnum[wp[i][j]] += 1
            if wp[i][j] in grads:
...
def rae_trainning_normal(wd_extra, neta = None, regu=0.9, w=None, wpresent=None):
    ...
    for iword in wd_extra:
        try:
            ...
            vect_, dw, epnum, grads = rae_operation(vect=vect, w=w, w_size=iword['w_size'], h_vect=iword['h_vect'], Word_ids= iword['Word_ids'], wp=iword['wp'], h_index=iword['h_index'], dep_order = iword['dep_order'], hh_index=iword['hh_index'], p=iword['p'], neta=neta, regu=regu, wpresent=wpresent)
            terr = 0.0
            bc=0
